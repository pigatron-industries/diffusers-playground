...
class MaskDilationNode(FunctionalNode):
    ...
    def process(self, mask:Image.Image, dilation:int, feather:bool) -> Image.Image:
        mask = mask.convert("L")
        mask = mask.filter(ImageFilter.MaxFilter(dilation*2+1))
        if feather:
            mask = mask.filter(ImageFilter.GaussianBlur(radius=dilation//2-1))
        return mask
