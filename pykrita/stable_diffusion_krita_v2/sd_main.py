...
class SDDialog(QDialog):
    def __init__(self,action,image):
        ...
        if (data["action"] not in ("upscale")):
            ...
            self.scale=self.addSlider(formLayout,data["scale"]*10,10,300,5,10)
        ...
        # scale_label=QLabel("Sampling method")
...
class showImages(QDialog):
    def __init__(self, imagedata, p: SDParameters):
        super().__init__(None)
        ...
        self.imgLabels=[0]*len(imagedata)
        self.seedLabel=[0]*len(imagedata)
        i=0
        for image in imagedata:       
            ...
            imgLabel.setPixmap(QPixmap.fromImage(image["image"]).scaled(380,380,Qt.KeepAspectRatio))  
            seedLabel=QLabel(image["seed"])
            ...
            btn.clicked.connect(lambda ch, num=i: selectImage(p, self.imagedata[num]["qimage"]))
    ...
    def updateImage(self, imagedata):
        ...
        self.imagedata[num]=imagedata["qimage"]
        imgLabel.setPixmap(QPixmap.fromImage(imagedata["qimage"]).scaled(380,380,Qt.KeepAspectRatio))  


def imageResultDialog(imagedata,p):
    dlg = showImages(imagedata,p)
    if dlg.exec():
  ...
  #  print(response)
    ...
    if (params.imageDialog):                 # only refresh image
        if (params.regenerate):
            print("generate new")
            params.imageDialog.updateImages(imagedata)
        else:  
            params.imageDialog.updateImage(imagedata[0])
    return imagedata
